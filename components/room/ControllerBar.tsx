...
interface ControlBarProps {
    ...
    roomId: string;
    session: Session;
    onShareRequest: (requester: { name: string; image: string }, requesterSocketId: string) => void;
}
    ...
    return (
        <div className="flex items-center justify-between px-6 py-4 border-t border-white/6 bg-[#0d0f14]/80 backdrop-blur-sm">
            ...
            <div className="flex items-center gap-2">
                ...
                <button
                    onClick={handleLeave}
                    className="flex items-center gap-2 bg-red-500/20 hover:bg-red-500/30 border border-red-500/40 text-red-400 rounded-xl px-4 h-10 text-sm font-medium transition-all duration-200 cursor-pointer"
                >
